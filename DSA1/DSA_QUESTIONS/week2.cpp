...
void Analysis(int n, int *arr1) {
    ...
    s3.display();
    cout << endl << "RESULT OF ANALYSIS" << endl;
      if(( swaps[0]<= swaps[1]) && (swaps[0] <= swaps[2]) &&
               (compares[0] <= compares[1]) && (compares[0] <= compares[2]))
            {
               cout<< "Bubble Sort was the best technique";
            }
            else if((swaps[1] <= swaps[0]) && (swaps[1] <= swaps[2]) &&
               (compares[1] <= compares[0]) && (compares[1] <= compares[2]))
            {
               cout<< "Insertion Sort was the best technique";
            }
            else if((swaps[2] <= swaps[0]) && (swaps[2] <= swaps[1]) &&
               (compares[2] <= compares[0]) && (compares[2] <= compares[1]))
            {
               cout<< "Selection Sort was the best technique";
            }
...
int main() {
    ...
    if (choice >= 1 && choice <= 3) {
        ...
        sortingObject.setarr(arr1);

        switch (choice) {
            case 1:
                sortingObject.setvalues();
                sortingObject.SelectionSort();
        ...
        cout << "Enter the number of elements you want to analyze" << endl;
        cin >> n;
        int arr1[n];
